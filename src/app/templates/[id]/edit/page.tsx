...
export default function EditTemplatePage() {
  ...
  const [editorMode, setEditorMode] = useState<EditorMode>('html'); // Default to HTML mode for editing
  ...
  useEffect(() => {
    const loadTemplate = async () => {
      try {
        ...
        // Set HTML content
        setCustomHtml(data.body_html || '');
        setGeneratedHtml(data.body_html || '');
      } catch {
    ...
    return (
      <DashboardLayout>
        <div className="flex items-center justify-center h-screen">
          <Loader2 className="w-8 h-8 animate-spin text-[#5B8C51]" />
        </div>
      </DashboardLayout>
    );
  }
  ...
  return (
    <DashboardLayout>
      <Header
        ...
        subtitle={template?.name || 'E-posta şablonunuzu düzenleyin'}
        action={
